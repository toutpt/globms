...
# Redirect to intranet member folder if it exists
if not next:
    ...
    try:
        portal.restrictedTraverse(('intranet', member_id))
        intranet_url = "%s/intranet/%s" % (portal.absolute_url(), member_id)
        return REQUEST.RESPONSE.redirect(intranet_url)
    except:
        pass
